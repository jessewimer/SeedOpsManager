Sub SetLbs()
    ...
    lbsAmt = Application.InputBox("Enter the number to deduct", Type:=1)
    ...
    Else
        cell.Select
        If Range("CD1") = 1 Then
            activeCell.Offset(, 8).value = lbsAmt
        ElseIf Range("CD1") = 2 Then
            activeCell.Offset(, 14).value = lbsAmt
        Else
            activeCell.Offset(, 20).value = lbsAmt
        End If
    End If
